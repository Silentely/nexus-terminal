...
describe('SSH Key Service', () => {
    ...
    describe('createSshKey', () => {
        ...
        it('缺少名称应抛出异常', async () => {
            await expect(
                createSshKey({ name: '', private_key: 'key' })
            ).rejects.toThrow('必须提供密钥名称和私钥内容。');
        });

        it('缺少私钥应抛出异常', async () => {
            await expect(
                createSshKey({ name: 'Test', private_key: '' })
            ).rejects.toThrow('必须提供密钥名称和私钥内容。');
        });
        ...
        it('名称重复时应抛出友好异常', async () => {
            ...
            await expect(
                createSshKey({ name: 'Duplicate', private_key: 'key' })
            ).rejects.toThrow('SSH 密钥名称 "Duplicate" 已存在。');
        });
    });
    ...
    describe('updateSshKey', () => {
        ...
        it('空私钥应抛出异常', async () => {
            ...
            await expect(
                updateSshKey(1, { private_key: '' })
            ).rejects.toThrow('私钥内容不能为空。');
        });
