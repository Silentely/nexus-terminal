...
export function registerUserSocket(userId: number, ws: AuthenticatedWebSocket): void {
    ...
    console.log(`[WebSocket 状态] 用户 ${userId} 的连接已注册，当前连接数: ${userSockets.get(userId)!.size}`);
}
...
export function broadcastToUser(userId: number, message: any): number {
    ...
    console.log(`[WebSocket 广播] 已向用户 ${userId} 的 ${successCount}/${sockets.size + deadSockets.length} 个连接发送消息。`);
    return successCount;
}
