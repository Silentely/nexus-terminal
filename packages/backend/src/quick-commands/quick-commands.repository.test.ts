...
describe('Quick Commands Repository', () => {
    ...
    describe('getAllQuickCommands', () => {
        it('应返回所有快捷指令（默认按名称排序）', async () => {
            const mockRows = [
                { id: 1, name: 'Alpha', command: 'cmd1', usage_count: 5, variables: '{"key":"value"}', created_at: 1000, updated_at: 1000, tag_ids_str: '1,2' },
                { id: 2, name: 'Beta', command: 'cmd2', usage_count: 3, variables: null, created_at: 1001, updated_at: 1001, tag_ids_str: null },
            ];
        ...
        it('应优雅处理 JSON 解析错误', async () => {
            const mockRows = [
                { id: 1, name: 'Test', command: 'cmd', usage_count: 0, variables: 'invalid json', created_at: 1000, updated_at: 1000, tag_ids_str: null },
            ];
            (allDb as any).mockResolvedValueOnce(mockRows);
