...
describe('appearance.repository', () => {
    ...
    describe('updateAppearanceSettings', () => {
        it('activeTerminalThemeId 指向不存在主题时应抛出异常', async () => {
            ...
            await expect(updateAppearanceSettings({ activeTerminalThemeId: 123 } as any)).rejects.toThrow('验证主题 ID 失败');
        });
        ...
        it('数据库写入错误时应抛出异常', async () => {
            ...
            await expect(updateAppearanceSettings({ terminalFontSize: 12 } as any)).rejects.toThrow('更新外观设置失败');
        });
    });
});
