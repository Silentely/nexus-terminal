...
describe('appearance.service', () => {
    ...
    it('updateSettings activeTerminalThemeId 类型非法时应抛错', async () => {
        ...
        await expect(service.updateSettings({ activeTerminalThemeId: '1' as any })).rejects.toThrow('无效的终端主题 ID 类型');
    ...
    it('updateSettings 指定不存在的终端主题时应抛错', async () => {
        ...
        await expect(service.updateSettings({ activeTerminalThemeId: 123 })).rejects.toThrow('验证终端主题 ID 时出错');
        expect(appearanceRepository.updateAppearanceSettings).not.toHaveBeenCalled();
    });

    it('updateSettings 终端背景蒙版透明度超出范围应抛错', async () => {
        const { service, appearanceRepository } = await loadService();
        await expect(service.updateSettings({ terminalBackgroundOverlayOpacity: 2 as any })).rejects.toThrow('无效的终端背景蒙版透明度');
        expect(appearanceRepository.updateAppearanceSettings).not.toHaveBeenCalled();
    });
    ...
    it('updateSettings remoteHtmlPresetsUrl 为空字符串时应归一化为 null', async () => {
        ...
        expect(ok).toBe(true);
        expect(appearanceRepository.updateAppearanceSettings).toHaveBeenCalledWith({ remoteHtmlPresetsUrl: null });
    });
    ...
    it('removeTerminalBackground 在无文件路径时不应删除文件但应清空数据库字段', async () => {
        ...
        expect(appearanceRepository.updateAppearanceSettings).toHaveBeenCalledWith({ terminalBackgroundImage: '' });
    });
    ...
    it('listRemoteHtmlPresets 成功时应仅返回远程 .html 文件', async () => {
        ...
        const list = await service.listRemoteHtmlPresets('https://github.com/user/repo/tree/main/themes');
    ...
    it('getRemoteHtmlPresetContent fileUrl 非 http/https 时应抛错', async () => {
        ...
        await expect(service.getRemoteHtmlPresetContent('file:///etc/passwd')).rejects.toThrow('HTTP/HTTPS');
    });
