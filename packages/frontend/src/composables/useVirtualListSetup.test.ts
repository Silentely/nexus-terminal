...
describe('useVirtualListSetup', () => {
  ...
  describe('overscan 精确边界值', () => {
    ...
    it('行高 25px 时 overscan 应为 ceil(200/25)=8', () => {
      const dataSource = ref([1]);
      useVirtualListSetup(dataSource, { itemHeight: 25 });
      // Math.ceil(200/25) = 8, clamp(8, 5, 15) = 8
      expect(useVirtualList).toHaveBeenCalledWith(
        dataSource,
    ...
    it('行高 100px 时 overscan 应为 ceil(200/100)=2，钳制到最小值 5', () => {
      const dataSource = ref([1]);
      useVirtualListSetup(dataSource, { itemHeight: 100 });
      // Math.ceil(200/100) = 2, clamp(2, 5, 15) = 5
      expect(useVirtualList).toHaveBeenCalledWith(
