...
describe('createWorkerPool', () => {
  ...
  describe('所有 Worker 忙碌时的队列行为', () => {
    it('单 Worker 忙碌时第二个任务提交不应立即发送 postMessage', async () => {
      const createWorkerPool = await getCreateWorkerPool();
      const pool = createWorkerPool(workerUrl, { size: 1, timeout: 5000 });

      // Submit first task — Worker becomes busy
      pool.execute<string>('task1', { n: 1 });
      expect(pool.hasIdle).toBe(false);
      expect(createdWorkers[0].postMessage).toHaveBeenCalledTimes(1);

      // Submit second task while Worker is busy — should queue, not dispatch immediately
      pool.execute<string>('task2', { n: 2 });

      // postMessage should still only have been called once (for task1)
      expect(createdWorkers[0].postMessage).toHaveBeenCalledTimes(1);

      pool.destroy();
    });

    it('Worker 忙碌时 hasIdle 应返回 false', async () => {
      const createWorkerPool = await getCreateWorkerPool();
      const pool = createWorkerPool(workerUrl, { size: 1 });

      // Submit task to make the single worker busy
      pool.execute('task', {});
      expect(pool.hasIdle).toBe(false);
    ...
    it('所有 Worker 忙碌时排队任务超时后应 reject', async () => {
      const createWorkerPool = await getCreateWorkerPool();
      const pool = createWorkerPool(workerUrl, { size: 1, timeout: 500 });
      ...
      // Second task queued — will timeout since worker stays busy
    ...
    it('两个 Worker 都忙碌时 hasIdle 应为 false', async () => {
      const createWorkerPool = await getCreateWorkerPool();
      const pool = createWorkerPool(workerUrl, { size: 2 });

      // Submit two tasks to occupy both workers
      pool.execute('task1', {});
      pool.execute('task2', {});
      expect(pool.hasIdle).toBe(false);
  ...
  describe('异步 fallback', () => {
    it('fallback 可以返回 Promise', async () => {
      const workerBackup = globalThis.Worker;
      // @ts-ignore
      delete globalThis.Worker;
      ...
      const result = await pool.execute('process', { data: 'test' });
      expect(result).toBe('async-fallback');
      expect(fallback).toHaveBeenCalledWith('process', { data: 'test' });
  ...
  describe('默认超时', () => {
    it('默认超时应为 30000ms', async () => {
      ...
      const executePromise = pool.execute('longTask', {});

      // 29999ms should not timeout yet
      vi.advanceTimersByTime(29999);
      // Still pending — verify promise hasn't rejected by checking hasIdle is still false
      expect(pool.hasIdle).toBe(false);

      // Advance past 30000ms to trigger timeout
