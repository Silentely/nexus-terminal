...
describe('createWorkerPool', () => {
  ...
  describe('异步 fallback 支持', () => {
    it('fallback 返回 Promise 时应正确解析结果', async () => {
      const savedWorker = globalThis.Worker;
      (globalThis as unknown as Record<string, unknown>).Worker = undefined;
      ...
      const result = await pool.execute<string>('task', { data: 'input' });

      expect(result).toBe('async-fallback');
      expect(fallback).toHaveBeenCalledWith('task', { data: 'input' });
  ...
  describe('payload 边界情况', () => {
    it('payload 为 null 时应正常发送给 Worker', async () => {
      const pool = createWorkerPool(workerUrl, { size: 1 });
      const worker = createdWorkers[0];
    ...
    it('payload 为 undefined 时应正常处理', async () => {
      const pool = createWorkerPool(workerUrl, { size: 1 });
      const worker = createdWorkers[0];

      const executePromise = pool.execute<string>('task', undefined);
      const sentMessage = worker.postMessage.mock.calls[0][0];
    ...
    it('payload 为复杂嵌套对象时应正常传递', async () => {
      const pool = createWorkerPool(workerUrl, { size: 1 });
      const worker = createdWorkers[0];

      const complexPayload = {
        nested: { arr: [1, 2, 3], map: { a: true } },
        text: 'hello',
        num: 42,
      };

      const executePromise = pool.execute<string>('task', complexPayload);
      const sentMessage = worker.postMessage.mock.calls[0][0];
  ...
  describe('默认超时', () => {
    it('默认超时为 30000ms（不会在 29999ms 内触发）', async () => {
      vi.useFakeTimers();

      const pool = createWorkerPool(workerUrl, { size: 1 }); // no explicit timeout

      const executePromise = pool.execute<string>('task', {});
      let rejected = false;
      executePromise.catch(() => { rejected = true; });

      vi.advanceTimersByTime(29999);
      // Should not have rejected yet
      expect(rejected).toBe(false);

      pool.destroy(); // clean up the pending promise
    });
  });

  describe('execute 消息结构', () => {
    it('发送的消息应包含唯一的 id 字段', async () => {
      const pool = createWorkerPool(workerUrl, { size: 2 });
      const worker1 = createdWorkers[0];
      const worker2 = createdWorkers[1];

      const p1 = pool.execute<string>('t1', {});
      const p2 = pool.execute<string>('t2', {});

      const id1 = worker1.postMessage.mock.calls[0][0].id;
      const id2 = worker2.postMessage.mock.calls[0][0].id;
      ...
      // Clean up
      worker1.simulateResponse({ id: id1, type: 't1', payload: 'r1' });
      worker2.simulateResponse({ id: id2, type: 't2', payload: 'r2' });
      await Promise.all([p1, p2]);

      pool.destroy();
    });

    it('消息 type 字段应与 taskType 参数一致', async () => {
      const pool = createWorkerPool(workerUrl, { size: 1 });
      const worker = createdWorkers[0];

      const p = pool.execute<string>('my-task-type', { foo: 'bar' });
      const msg = worker.postMessage.mock.calls[0][0];
      expect(msg.type).toBe('my-task-type');

      worker.simulateResponse({ id: msg.id, type: 'my-task-type', payload: 'ok' });
      await p;
