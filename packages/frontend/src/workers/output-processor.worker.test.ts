...
describe('LOG HTTP 状态码高亮', () => {
  ...
  it('HTTP 2xx 状态码应高亮为绿色', () => {
    const response = sendMessage('process', {
      text: '2024-01-15 INFO: GET /api/users returned 200 OK',
      options: { enableLinkDetection: false },
    });
    expect(response.payload.type).toBe('log');
    expect(response.payload.content).toContain('\x1b[32m'); // GREEN
  ...
  it('HTTP 3xx 状态码应高亮为青色', () => {
    const response = sendMessage('process', {
      text: '2024-01-15 INFO: GET /old-path returned 301 Moved',
      options: { enableLinkDetection: false },
    });
    expect(response.payload.type).toBe('log');
    expect(response.payload.content).toContain('\x1b[36m'); // CYAN
  ...
  it('HTTP 4xx 状态码应高亮为黄色', () => {
    const response = sendMessage('process', {
      text: '2024-01-15 ERROR: GET /api/missing returned 404 Not Found',
      options: { enableLinkDetection: false },
    });
    expect(response.payload.type).toBe('log');
    expect(response.payload.content).toContain('\x1b[33m'); // YELLOW
...
describe('configure 部分合并', () => {
  ...
  it('configure 空对象 {} 应返回 ok 并不改变配置', () => {
    const response = sendMessage('configure', {}, 'cfg-empty');
    expect(response.payload).toEqual({ ok: true });
  });

  it('configure 只更新 foldThreshold 时 enableHighlight 应保持不变', () => {
    // Disable highlighting
    sendMessage('configure', { enableHighlight: false }, 'cfg-1');
    postMessageMock.mockClear();

    // Update only foldThreshold
    sendMessage('configure', { foldThreshold: 300 }, 'cfg-2');
    postMessageMock.mockClear();

    // Verify enableHighlight is still false (config merged, not replaced)
    const response = sendMessage('process', { text: '{"key": "val"}' }, 'proc');
    expect(response.payload.content).not.toContain('\x1b[');

    // Reset for other tests
    sendMessage('configure', { enableHighlight: true }, 'reset-hl');
  });

  it('configure 应返回与请求相同的 id', () => {
    const response = sendMessage('configure', { foldThreshold: 200 }, 'my-cfg-id');
    expect(response.id).toBe('my-cfg-id');
    expect(response.type).toBe('configure');
    expect(response.payload).toEqual({ ok: true });
  ...
  it('process 内联 options 应优先于 configure 全局配置', () => {
    // Set global highlight to false
    sendMessage('configure', { enableHighlight: false }, 'cfg');
    postMessageMock.mockClear();

    // But override with per-request options
    const response = sendMessage('process', {
      text: '{"key": "value"}',
    ...
    expect(response.payload.type).toBe('json');
    // enableHighlight=true in options means ANSI codes present
    expect(response.payload.content).toContain('\x1b[');

    // Reset
    sendMessage('configure', { enableHighlight: true }, 'reset');
  });
});
...
describe('TABLE 格式化', () => {
  ...
  it('禁用 TABLE 格式化时内容不应包含对齐 ANSI 码', () => {
    ...
    expect(response.payload.type).toBe('table');
    // When table formatting is disabled, no cyan+bold header coloring
    expect(response.payload.content).not.toContain('\x1b[36m'); // No CYAN from table header
  });

  it('启用 TABLE 格式化时表头应高亮为青色粗体', () => {
    const tableText = '| Name   | Age   |\n| Alice  | 30    |\n| Bob    | 25    |';
    const response = sendMessage('process', {
      text: tableText,
    ...
    expect(response.payload.type).toBe('table');
    expect(response.payload.content).toContain('\x1b[36m'); // CYAN (table header)
    expect(response.payload.content).toContain('\x1b[1m');  // BOLD (table header)
  });
});
