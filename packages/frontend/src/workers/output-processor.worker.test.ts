...
describe('output-processor.worker', () => {
  ...
  describe('5000 行边界条件', () => {
    it('恰好 5000 行应正常高亮处理（不跳过）', () => {
      ...
      // Should not be TEXT due to large-file protection (5001 threshold)
      // 5000 lines: text content with no JSON/YAML/LOG markers → TEXT type
      expect(response.payload.type).toBe('text');
      // But importantly, it should NOT be the short-circuit path: content is processed normally
      expect(response.error).toBeUndefined();
    });

    it('5001 行应跳过高亮（大文件保护）并返回 text 类型', () => {
      const text5001 = Array.from({ length: 5001 }, (_, i) => `line ${i}`).join('\n');
    ...
    it('5000 行 LOG 内容应正常高亮（未超出阈值）', () => {
      const logLines = Array.from(
        { length: 5000 },
        (_, i) => `2024-01-15 10:30:${String(i % 60).padStart(2, '0')} INFO message ${i}`
      );
      const response = sendMessage('process', { text: logLines.join('\n') });
  ...
  describe('JSON 错误降级', () => {
    it('JSON 检测通过但 parse 失败时应降级到其他格式', () => {
      // Looks like JSON (starts with {) but is invalid
      const response = sendMessage('process', { text: '{key: value}' });

      // Should not return json type since JSON.parse fails
