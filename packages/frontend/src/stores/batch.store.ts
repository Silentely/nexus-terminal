...
export const useBatchStore = defineStore('batch', () => {
    ...
    // === Getters ===
    const hasActiveTask = computed(() => currentTask.value !== null && currentTask.value.status === 'in-progress');
        ...
        switch (type) {
            case 'batch:subtask:update':
                ...
                if (payload.subTaskId) {
                    const subTask = currentTask.value.subTasks.find((st) => st.subTaskId === payload.subTaskId);
                    if (subTask) {
            ...
            case 'batch:overall':
                ...
                if (payload.overallProgress !== undefined) currentTask.value.overallProgress = payload.overallProgress;
                if (payload.completed !== undefined) currentTask.value.completedSubTasks = payload.completed;
