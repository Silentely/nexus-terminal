<template>
  <div class="p-4 bg-background text-foreground h-full flex flex-col"> <!-- Full height flex container -->
    <div class="max-w-7xl mx-auto w-full flex flex-col h-full"> 
      <h1 class="text-xl font-semibold text-foreground mb-4 pb-2 border-b border-border flex-shrink-0">
        {{ $t('auditLog.title') }}
      </h1>

      <!-- Filtering Controls -->
      <div class="flex flex-wrap items-center gap-4 mb-4 p-4 border border-border rounded-lg bg-header/50 flex-shrink-0">
        ...
        <div class="flex-grow min-w-[200px]">
          ...
          <select id="action-type" v-model="selectedActionType"
                  ...
                  style="background-image: url('data:image/svg+xml,%3csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 16 16\'%3e%3cpath fill=\'none\' stroke=\'%236c757d\' stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M2 5l6 6 6-6\'%2f%3e%3c/svg%3e'); background-position: right 0.75rem center; background-size: 16px 12px;">
      ...
      <!-- Error state -->
      <div v-if="store.error" class="p-4 mb-4 border-l-4 border-error bg-error/10 text-error rounded flex-shrink-0">
        {{ store.error }}
      </div>

      <!-- Loading state -->
      <div v-else-if="store.isLoading && logs.length === 0" class="p-4 text-center text-text-secondary italic flex-shrink-0">
        {{ $t('common.loading') }}
      </div>

      <!-- No logs state -->
      <div v-else-if="!store.isLoading && !store.error && logs.length === 0" class="p-4 mb-4 border-l-4 border-blue-400 bg-blue-100 text-blue-700 rounded flex-shrink-0">
        {{ $t('auditLog.noLogs') }}
      </div>

      <!-- Virtual List -->
      <div v-else-if="!store.isLoading && !store.error && logs.length > 0" class="flex-grow flex flex-col min-h-0 border border-border rounded-lg bg-background">
          <!-- Header Row -->
          <div class="flex bg-header border-b border-border text-sm font-medium text-text-secondary py-3 px-4 flex-shrink-0">
          ...
          <div v-bind="containerProps" class="flex-grow overflow-auto custom-scrollbar">
            <div v-bind="wrapperProps">
              <div v-for="{ data: log, index } in list" :key="log.id" 
                   class="flex border-b border-border hover:bg-header/50 text-sm py-4 px-4 items-start"
                   :style="{ minHeight: '60px' }">
                <div class="w-1/6 min-w-[150px] pr-4 break-words">{{ formatTimestamp(log.timestamp) }}</div>
                <div class="w-1/6 min-w-[150px] pr-4 break-words">{{ translateActionType(log.action_type) }}</div>
                <div class="w-4/6 flex-grow min-w-0">
                  <pre v-if="log.details" class="whitespace-pre-wrap break-all bg-header/50 p-2 border border-border/50 rounded text-xs font-mono max-h-40 overflow-y-auto">{{ formatDetails(log.details) }}</pre>
          ...
          <div class="text-right text-text-secondary text-sm p-4 border-t border-border flex-shrink-0 bg-header/20">
              ...
              <div class="inline-flex ml-4 gap-2">
                <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1" class="px-2 py-1 border border-border rounded disabled:opacity-50 hover:bg-header">&lt;</button>
                <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages" class="px-2 py-1 border border-border rounded disabled:opacity-50 hover:bg-header">&gt;</button>
              </div>
          </div>
      </div>
    </div>
