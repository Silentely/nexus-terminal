...
- ✨ NoMore Spam 垃圾信息检测工作流并移除旧的 AI Issue 智能回复工作流
- ✨ 10 轮打磨迭代——i18n 补齐、日志脱敏、UI 统一与性能优化
- ✨ 组件加载态统一、字节格式化去重与前后端日志收口
- ✨ 轮询可见性感知、Docker 命令失败反馈与前端文案本地化
- ✨ 完善批量执行取消流程与 Worker 池生命周期处理
- ✨ 统一日期与防抖工具，新增错误横幅并优化通知体验
- ⏱️ 提取共享延迟工具（含可取消版本），统一 apiClient 重试等处的重复实现
- 🎨 style.css 新增 @layer components 语义类 `.btn-primary`/`.form-input`，设置页主按钮与验证码表单输入框接入；上下文菜单键盘支持
- ♿ 33 处仅图标按钮补全 aria-label；对话框安全 id 与焦点管理；10 个模态框 role=dialog；批量执行面板补充键盘操作、进度条和状态 live region、可见焦点样式及 reduced-motion 支持
- 📊 图表颜色主题化修复与响应式 CSS 变量工具
- 🔇 前端日志输出统一收口；App.vue 键盘热路径日志降噪
- 🔤 前端英文日志全面统一为中文；后端 i18n.ts console 统一为惰性 pino logger
- 💬 sudo 执行和批量取消统一使用确认对话框；复制输出会显示成功或失败通知
- 🔄 Docker 管理器 15 秒状态轮询与批量执行降级轮询增加页面可见性感知
- 🐳 Docker 容器操作失败现在会通过通知反馈用户
- ✍️ 批量编辑连接与标签管理弹窗、远程桌面/VNC 通道错误兜底文案改为本地化提示
- 📧 邮件通知默认主题统一为 `[Nexus Terminal]` 前缀风格

### 修复

- 🔒 提升 tar 依赖至 7.5.21+ 修复生产依赖 critical 漏洞
- 🐛 完成 ja-JP 语言包中文残留翻译，清零遗留债务
- 🐛 消除 i18n 守卫测试的 ESLint 告警
- 🔧 修复发布流水线标签漏打与 AI 兜底崩溃两处缺陷
- 📊 修复 Chart.js 图表颜色不随主题切换的问题
- 🔗 批量 WebSocket 终态事件完整通过 parser、当前 SSH 会话和 Batch Store；非终态事件不会提前关闭 REST 轮询兜底
- 🚦 批量 SSH 输出增加内存、数据库和 WebSocket 单次载荷边界
- 🧵 Worker 超时或运行时错误后终止并替换旧实例，销毁后的终端输出 Worker 池支持安全重建
- 🛑 取消操作先中断活动 SSH 命令，再写入排队子任务状态
- ✅ 后端日志全面达标：logger 前缀模块化一致、英文残留为 0
- 🧹 移除未使用导入修复 quality gate lint 失败

### 测试

- 🧪 后端核心控制器与服务测试补强
- 🧪 为 9 个核心 composable 补齐单元测试
- 🧪 修复 NL2CMD 单元测试依赖真实网络的问题
- ✅ 修复 SSH 服务延迟断言在计时器精度下偶发失败的问题
- 🧪 增加批量取消竞态、SSH 输出边界、Worker 池超时/error/destroy、Store 终态降级和批量面板交互测试
- 🧪 补充 Docker 管理器可见性感知轮询、命令失败反馈、批量降级轮询暂停/恢复等测试用例

### 杂项

- 🔧 镜像构建切换 dev/main 双分支策略
