...
- 🛑 取消操作现在先中断活动 SSH 命令，再写入排队子任务状态；进度写回会在父任务终态落盘前串行完成。
- ♿ 批量执行面板补充键盘操作、进度条和状态 live region、输出对话框语义、可见焦点样式及 reduced-motion 支持。
- 💬 sudo 执行和批量取消统一使用确认对话框；复制输出会显示成功或失败通知，中文、英文和日文文案同步更新。
- 🔄 Docker 管理器 15 秒状态轮询与批量执行降级轮询增加页面可见性感知：标签页隐藏时暂停请求，恢复可见后立即刷新并继续。
- 🐳 Docker 容器操作失败（启动/停止/重启/移除）现在会通过通知反馈用户，不再仅记录日志；Docker 面板「未知 SSH 错误」与无数据占位（N/A）改为本地化文案。
- ✍️ 批量编辑连接与标签管理弹窗的「Unknown error」、远程桌面/VNC 通道错误兜底文案改为本地化提示；26 处前端英文错误日志统一为中文。
- 📧 邮件通知默认主题统一为 `[Nexus Terminal]` 前缀风格，与主通知链路保持一致。
