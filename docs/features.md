...
- **多目标执行** — 同时向多台服务器发送命令
- **并行/串行** — 支持并行或按顺序执行
- **实时状态** — 显示每台服务器的排队、连接、执行、成功、失败和取消状态
- **取消闭环** — 取消时保留已完成/失败计数，并显示取消原因；排队中的目标不会继续执行
- **结果聚合** — 汇总所有服务器的执行结果，终态事件丢失时使用状态查询兜底
- **输出保护** — 服务端限制单次输出和持久化输出，前端限制单个目标的内存输出并显示截断提示
- **键盘与无障碍** — 服务器选择、命令输入、进度条、结果列表和输出对话框支持语义化操作
- **模板保存** — 常用命令模板快速调用
...
- **终端输出处理** — 提供可选 Worker 异步处理 API；xterm 热路径保持有序同步处理，避免异步结果乱序
- **通用 Worker 池** — createWorkerPool 管理多 Worker 并行任务
- **主线程降级** — Worker 不可用时自动回退到同步处理
- **Promise API** — 基于 ID 的请求/响应关联，支持超时控制
- **失效替换** — Worker 超时或 error 后自动终止并创建替代实例，迟到响应不会影响后续任务
- **生命周期回收** — 池销毁时拒绝排队/执行中的请求；终端输出处理池可在销毁后安全重新初始化
